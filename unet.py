...
model=UNet(); opt=optim.Adam(model.parameters(), lr=1e-3); loss=nn.BCELoss()
for ep in range(epochs):
    for x,_ in dataloader:
        yhat=model(x); y=torch.ones_like(yhat) # dummy seg mask
        l=loss(yhat,y); opt.zero_grad(); l.backward(); opt.step()
